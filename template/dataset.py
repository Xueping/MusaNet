...
from utils.icd9_ontology import convert_to_3digit_icd9
...
class DatasetTemplate(metaclass=ABCMeta):
    ...
    def build_dictionary(self):
        ...
        for code in all_codes:
            digit = convert_to_3digit_icd9(code[2:])
            all_3digit_codes.append(digit)
        count_3digit_codes = []
        count_3digit_codes.extend(collections.Counter(all_3digit_codes).most_common())
        count_3digit = []
        for word, c in count_3digit_codes:
            word_tuple = [word, c]
            if c >= self.min_freq:
                count_3digit.append(word_tuple)
        for word, cnt in count_3digit:
